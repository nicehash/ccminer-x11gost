/**
 * Optimized Blake-256 8round Cuda Kernel (Tested on SM >3.0)
...
void blake256_gpu_hash_16_8(const uint32_t threads, const uint32_t startNonce, uint32_t *resNonce,const uint32_t highTarget){
	...
	const uint32_t c_u256[16] = {	0x243F6A88, 0x85A308D3, 0x13198A2E, 0x03707344,	0xA4093822, 0x299F31D0, 0x082EFA98, 0xEC4E6C89,
...
__host__
void blake256_cpu_setBlock_16(const uint32_t* endiandata, uint32_t *penddata){
	...
	/* get message */
	m.u128[0] = LOADU(endiandata + 0);	m.u128[1] = LOADU(endiandata + 4);	m.u128[2] = LOADU(endiandata + 8);	m.u128[3] = LOADU(endiandata + 12);
...
extern "C" int scanhash_blake256(int thr_id, struct work* work, uint32_t max_nonce, unsigned long *hashes_done,const int8_t blakerounds){
	...
	const uint32_t intensity	= (device_sm[device_map[thr_id]] > 500) ? 30 : 28;
	const uint32_t throughput	= min(device_intensity(device_map[thr_id], __func__, 1U << intensity), max_nonce - first_nonce);
	...
	blake256_cpu_setBlock_16(endiandata,&pdata[16]);

	cudaMemset(d_resNonce[thr_id], 0xff, sizeof(uint32_t));
	const dim3 grid((throughput + (NPT*TPB)-1)/(NPT*TPB));
	const dim3 block(TPB);
	do {
		blake256_gpu_hash_16_8<<<grid,block>>>(throughput, pdata[19], d_resNonce[thr_id], targetHigh);
